...
Encryption and decryption use the **same** XOR with the same IV-derived keystream. Unlike [Camellia-CTR](./camellia_ctr), successive keystream blocks depend on a **chain** of Camellia encryptions on the register, not on incrementing a counter. There is **no** authentication tag.

## Purpose

Use **Camellia-OFB** when a legacy profile requires it. For new work that needs integrity, prefer an **AEAD** such as [AES-GCM](./aes_gcm) or [ChaCha20-Poly1305](./chacha20_poly1305).
