...
fn main() {
    ...
    let app_record = conn
        .noxtls_seal_record(b"demo-application-data", b"tls13-header")
        .expect("record seal should succeed");
    let app_plaintext = conn
        .noxtls_open_own_record(&app_record, b"tls13-header")
    ...
    let fragments = conn
        .noxtls_seal_record_fragments(fragment_payload, b"tls13-header", 10)
        .expect("fragmented seal should succeed");
    let reassembled = conn
        .noxtls_open_own_record_fragments(&fragments, b"tls13-header")
    ...
    if tampered.len() > 1 {
        ...
        let err = conn
            .noxtls_open_own_record_fragments(&tampered, b"tls13-header")
...
/// # Panics
///
/// Panics if any intermediate `noxtls_send_client_hello`, `noxtls_recv_server_hello`, secret derivation, or `noxtls_finish` step fails.
fn establish_tls13_session() -> Connection {
    let mut conn = Connection::noxtls_new(TlsVersion::Tls13);
    let client_hello = conn
        .noxtls_send_client_hello(&[0x42; 32])
    ...
    conn.noxtls_finish(&verify).expect("noxtls_finish should succeed");
...
fn run_tls13_certificate_auth_failure_scenarios() {
    ...
    let certificate_msg = Connection::noxtls_build_certificate_message(&malformed_leaf)
        .expect("certificate message should build");

    let mut missing_anchors = establish_tls13_post_encrypted_extensions();
    missing_anchors.noxtls_set_tls13_require_certificate_auth(true);
    let missing_anchors_err = missing_anchors
        .noxtls_recv_certificate(&certificate_msg)
    ...
    malformed_chain.noxtls_set_tls13_require_certificate_auth(true);
    malformed_chain
        .noxtls_configure_tls13_server_auth(&[vec![0x30, 0x00]], &[], "20250101000000Z")
    ...
    cert_verify_chain.noxtls_set_tls13_require_certificate_auth(true);
    cert_verify_chain.state = noxtls::HandshakeState::ServerCertificateReceived;
    let cert_verify_msg = Connection::noxtls_build_certificate_verify_message(0x0804, &[0xAA, 0xBB, 0xCC])
        .expect("certificate verify message should build");
    let cert_verify_err = cert_verify_chain
        .noxtls_recv_certificate_verify(&cert_verify_msg)
...
fn establish_tls13_post_encrypted_extensions() -> Connection {
    let mut conn = Connection::noxtls_new(TlsVersion::Tls13);
    conn.noxtls_send_client_hello(&[0x77; 32])
        .expect("client hello should succeed");
    let server_hello = Connection::noxtls_build_server_hello(
        TlsVersion::Tls13,
        CipherSuite::TlsAes256GcmSha384,
    ...
    conn.noxtls_recv_server_hello(&server_hello)
        .expect("server hello should succeed");
    let encrypted_extensions = Connection::noxtls_build_encrypted_extensions();
    conn.noxtls_recv_encrypted_extensions(&encrypted_extensions)
